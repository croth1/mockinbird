    ...
    if args.bed_file:
        ...
        with open(args.bed_file) as bed_file:
            for line in bed_file:
                chrom, start_str, end_str, _, _, strand, *_ = line.split()
                if chrom == region.seqid:
                    iv = Interval(int(start_str) + 1, int(end_str))
                    if strand == '+':
                        plus_tree.insert(iv)
                    else:
                        minus_tree.insert(iv)
    ...
    with Popen([pileup_cmd % (args.genome_fasta, args.protein_bam, region)], **extra_args) as factor_proc,\
        ...
        while factor_line:
            ...
            for site in output_sites:
                tree = plus_tree if site.strand == '+' else minus_tree
                if tree and tree.has_overlap(site):
